...
def usage():
    ...
    print(
        ">python demo2d.py -n=8 [-c|--cov][-s|--steps=3000][-S=1234|--seed=1234][-x|--xstat][-p|--p00][-g|--gauss][-k|--k5x5][-d|--data][-o|--out]"
    )
...
def main():
    ...
    im = np.load(data)
    im = np.sum(
        np.sum(im.reshape(nside, im.shape[0] // nside, nside, im.shape[0] // nside), 3),
        1,
    )
    im = im / im.std()
