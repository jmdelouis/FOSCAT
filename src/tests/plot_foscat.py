...
try:
    ref = np.mean(
        np.load(datapath + "%s_REF.npy" % (sys.argv[1])).reshape(
            12 * nside**2, (256 // nside) ** 2
        ),
        1,
    )
except:
...
for i in range(len(tab)):
    mm = np.mean(
        np.load(scratch_path + "/MASK_GAL%s_256.npy" % (tab[i])).reshape(
            12 * nside**2, (256 // nside) ** 2
        ),
        1,
    )[idx]

    mm = hp.smoothing(mm, 5 / 180.0 * np.pi)
