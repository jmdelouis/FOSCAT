...
for itest in range(len(tabtype)):
    for k in range(6):
        ...
        print('Test Synthesis no cross 0 : Type=%s %s DONE'%(tabtype[itest],sc.__name__))
        ...
        if len(ims.shape)==1:
            im=np.random.randn(ims.shape[0])+complex(0,1)*np.random.randn(ims.shape[0])
        else:
            im=np.random.randn(ims.shape[0],ims.shape[1])+complex(0,1)*np.random.randn(ims.shape[0],ims.shape[1])
        ...
        print('Start Test COMPLEX Synthesis no cross : Type=%s %s ....'%(tabtype[itest],sc.__name__))
        ...
        if len(ims.shape)==1:
            imap=np.random.randn(ims.shape[0])+complex(0,1)*np.random.randn(ims.shape[0])
        else:
            imap=np.random.randn(ims.shape[0],ims.shape[1])+complex(0,1)*np.random.randn(ims.shape[0],ims.shape[1])
        
        omap=sy.run(imap,NUM_EPOCHS = 10)
        
        print('Test Synthesis COMPLEX no cross : Type=%s %s DONE'%(tabtype[itest],sc.__name__))
