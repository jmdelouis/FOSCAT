...
fc=FOC.FoCUS(healpix=False,NORIENT=4,KERNELSZ=3,OSTEP=0,TEMPLATE_PATH=scratch_path,slope=2)
...
for i in range(16):
    for j in range(ngrad):
        g22[i+16*j]=fc.opti.compute_gradients(s2[j][0,i//4,i%4],var_list=[fc.param[0]])
...
for itt in range(10000):
    ...
    for i in range(5):
        for j in range(4):
            tmp=(rr1[i][0,j]-r1[i][0,j])*(gg1[i*4+j][0][0].reshape(64,64)/(64*64))
            diff[0,:,:,0]+=tmp/64 #(tmp[xidx[i],yidx[i]].reshape(64,64)/64
    
    for i in range(ngrad):
        for j in range(16):
            tmp=(rr2[i][0,j//4,j%4]-r2[i][0,j//4,j%4])*(gg2[i*16+j][0][0].reshape(64,64))/(64*64)
...
for itt in range(1000):
    ...
    """
    for i in range(5):
        tmp=np.sum(((rr1[0,0,0,i*4:(i+1)*4]-r1[0,0,0,i*4:(i+1)*4]).reshape(1,1,4))*(gg[i][0,:,:,:]/(64*64)),2)
...
for itt in range(10):
    ...
    for i in range(20):
        ...
        vec+=(rr1[0,0,0,i]-r1[0,0,0,i])*imm/(20*64*64)*(2**(i//4)/16)
