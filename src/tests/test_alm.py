...
for BACKEND in bk_tab:
    for nside in nside_tab:
        ...
        cl1,cl1_l1=alm1.anafast(im,nest=True)
        cl1=alm1.backend.to_numpy(cl1)
        cl3=hp.anafast(hp.reorder(im,n2r=True),iter=0)
        ...
        cl1,cl1_l1=alm1.anafast(im[1:],nest=True)
        cl1=alm1.backend.to_numpy(cl1)
        cl3=hp.anafast(hp.reorder(im,n2r=True),iter=0)
        ...
        cl1,cl1_l1=alm1.anafast(im[0],nest=True)
        cl1=alm1.backend.to_numpy(cl1)
        cl3=hp.anafast(hp.reorder(im[0],n2r=True),iter=0)
...
for BACKEND in bk_tab:
    for nside in nside_tab:
        ...
        cl1,cl1_l1=alm1.anafast(im,axes=1,nest=True)
        cl1=alm1.backend.to_numpy(cl1)
        cl3=0*cl1
        print('TT,EE,BB,TE,EB,TB',nside,BACKEND)
        for l in range(3):
            cl3=hp.anafast(hp.reorder(im[l],n2r=True),iter=0)
            for k in range(6):
                print('foscat ml',l,(cl1[l,k]-cl3[k]).std())
                assert  (cl1[l,k]-cl3[k]).std()<1E-6/nside

        cl1,cl1_l1=alm1.anafast(im[:,1:],axes=1,nest=True)
        cl1=alm1.backend.to_numpy(cl1)
        ...
        cl1,cl1_l1=alm1.anafast(im[:,0],axes=1,nest=True)
        cl1=alm1.backend.to_numpy(cl1)
        print('TT',nside,BACKEND)
        for l in range(3):
            cl3=hp.anafast(hp.reorder(im[l,0],n2r=True),iter=0)
