...
for BACKEND in bk_tab:
    ...
    cmat_0, cmat2_0 = scat_op.stat_cfft(
        norm_noise[0], upscale=KERNELSZ == 5, smooth_scale=1
    )
    cmat_1, cmat2_1 = scat_op.stat_cfft(
        norm_noise[1], upscale=KERNELSZ == 5, smooth_scale=1
    )
    ...
    for k in range(heal_im.shape[0]):
        ref1[k] = scat_op.eval(
            (heal_im[k, 0] - mean_data[0]) / std_data[0],
            norm="self",
            cmat=cmat_0,
            cmat2=cmat2_0,
        )
        ref2[k] = scat_op.eval(
            (heal_im[k, 1] - mean_data[1]) / std_data[1],
            norm="self",
            cmat=cmat_1,
            cmat2=cmat2_1,
        )
    ...
    def The_loss_spec(x, scat_operator, args):
        ...
        loss = scat_operator.backend.bk_reduce_mean(
            scat_operator.backend.bk_square(c_l1 / d_c_l1)
        ) + scat_operator.backend.bk_reduce_mean(
            scat_operator.backend.bk_square(c_l2 / d_c_l2)
        )
        return loss
    ...
    def The_loss(x, scat_operator, args):
        ...
        loss = scat_operator.reduce_mean(
            scat_operator.square((learn - x.shape[0] * ref1) / sref1)
        )

        learn = scat_operator.eval(x[:, 1], norm="self", cmat=cmat11, cmat2=cmat12)
        learn = scat_operator.reduce_sum_batch(learn)
        loss = loss + scat_operator.reduce_mean(
            scat_operator.square((learn - x.shape[0] * ref2) / sref2)
        )

        return loss * 10

    loss = synthe.Loss(
        The_loss, scat_op, mref1, vref1, mref2, vref2, cmat_0, cmat2_0, cmat_1, cmat2_1
    )

    loss_sp = synthe.Loss(
        The_loss_spec,
        scat_op,
        scat_op.backend.bk_cast(mean_data),
        scat_op.backend.bk_cast(std_data),
        scat_op.backend.bk_cast(r_c_l1),
        scat_op.backend.bk_cast(r_c_l2),
        scat_op.backend.bk_cast(d_c_l1),
        scat_op.backend.bk_cast(d_c_l2),
        alm,
    )
    ...
    omap = scat_op.backend.to_numpy(sy.run(omap, EVAL_FREQUENCY=10, NUM_EPOCHS=100))
