...
if len(sys.argv) < 5:
    ...
    print("============================================")
    print(
        "<in>           : name of the 3 input data files: <in>_MONO.npy,<in>_HM1_MONO.npy,<in>_HM2_MONO.npy"
    )
    print(
        "<scratch_path> : name of the directory with all the input files (noise, TT,etc.) and also use for FOSCAT temporary files"
    )
...
tab = [
    "MASK_GAL11_%d.npy" % (nin),
    "MASK_GAL09_%d.npy" % (nin),
    "MASK_GAL08_%d.npy" % (nin),
    "MASK_GAL06_%d.npy" % (nin),
    "MASK_GAL04_%d.npy" % (nin),
]
...
ampmap = (
    ...
    / dodown(
        np.load(
            scratch_path + "%s_NOISE%03d_full.npy" % (sys.argv[1][0:6], 0)
        ).flatten(),
        nout,
    ).std()
)
...
for i in range(nsim):
    noise[i] = ampmap * dodown(
        np.load(
            scratch_path + "%s_NOISE%03d_full.npy" % (sys.argv[1][0:6], i)
        ).flatten(),
        nout,
    )
    noise1[i] = ampmap * dodown(
        np.load(
            scratch_path + "%s_NOISE%03d_hm1.npy" % (sys.argv[1][0:6], i)
        ).flatten(),
        nout,
    )
    noise2[i] = ampmap * dodown(
        np.load(
            scratch_path + "%s_NOISE%03d_hm2.npy" % (sys.argv[1][0:6], i)
        ).flatten(),
        nout,
    )
...
for itt in range(5):
    ...
    if rank == 0 or rank == 2 or size == 1:
        ...
        stat1_p_noise = scat_op.eval(
            i1 + noise1[0], image2=i2 + noise2[0], mask=mask, Auto=True
        )
        ...
        for k in range(1, nsim):
            stat1_p_noise = scat_op.eval(
                i1 + noise1[k], image2=i2 + noise2[k], mask=mask, Auto=True
            )
    ...
    if rank == 1 or size == 1:
        ...
        for k in range(1, nsim):
            stat2_p_noise = scat_op.eval(
                td, image2=imap + noise[k], mask=mask, Auto=False
            )
    ...
    if rank == 2 or size == 1:
        loss3 = synthe.Loss(
            loss_fct3, scat_op, refH - bias1, di.astype(dtype), bias3, mask, isig3, True
        )
        loss4 = synthe.Loss(
            loss_fct4, scat_op, di.astype(dtype), sig_noise, Rformat=False
        )
