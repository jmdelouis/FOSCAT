...
fc = FOC.FoCUS(
    NORIENT=4, KERNELSZ=3, healpix=True, OSTEP=0, slope=1.2, TEMPLATE_PATH=scratch_path
)
...
for itt in range(10):
    ...
    if fc.get_rank() == 2 % fc.get_size():
        ...
        if fc.get_size() == 1:
            ii = 2
        tb1[ii] = (
            Alpha * (np.mean(o1 - r1, 0) - np.mean(of1 - r1, 0) - tb1[ii]) + tb1[ii]
        )
        tb2[ii] = (
            Alpha * (np.mean(o2 - r2, 0) - np.mean(of2 - r2, 0) - tb2[ii]) + tb2[ii]
        )
