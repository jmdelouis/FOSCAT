...
def usage():
    ...
    print(
        ">python demo.py -n=8 [-c|--cov][-s|--steps=3000][-S=1234|--seed=1234][-x|--xstat][-p|--p00][-g|--gauss][-k|--k5x5][-d|--data][-o|--out][-K|--k128][-r|--orient]"
    )
...
def main():
    ...
    if (
        nside < 2
        or nside != 2 ** (int(np.log(nside) / np.log(2)))
        or (nside > 256 and KERNELSZ <= 5)
        or (nside > 2**instep and KERNELSZ > 5)
    ):
        print(
            "nside should be a power of 2 and in [2,...,256] or [2,...,%d] if -K|-k128 option has been choosen"
            % (2**instep)
        )
        usage()
        exit(0)
    ...
    def lossX(x, scat_operator, args):
        ...
        if dop00:
            loss = scat_operator.bk_reduce_mean(
                scat_operator.bk_square(ref.P00[0, 0, :] - learn.P00[0, 0, :])
            )
        else:
