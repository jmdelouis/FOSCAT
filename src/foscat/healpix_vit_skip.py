...
class HealpixViTSkip(nn.Module):
    def __init__(
        ...
        *,
        ...
        for d in self.level_dims:
            if d % self.G != 0:
                raise ValueError(
                    f"All level_dims must be divisible by G={self.G}, got {d}."
                )
        ...
        self.nsides_levels = [
            self.in_nside // (2**i) for i in range(self.token_down + 1)
        ]
        self.ntokens_levels = [12 * n * n for n in self.nsides_levels]

        # Patch embed (double conv)
        fine_g = self.C_fine // self.G
        self.pe_w1 = nn.Parameter(
            torch.empty(self.n_chan_in, fine_g, self.KERNELSZ * self.KERNELSZ)
        )
        nn.init.kaiming_uniform_(
            self.pe_w1.view(self.n_chan_in * fine_g, -1), a=np.sqrt(5)
        )
        self.pe_w2 = nn.Parameter(
            torch.empty(self.C_fine, fine_g, self.KERNELSZ * self.KERNELSZ)
        )
        nn.init.kaiming_uniform_(
            self.pe_w2.view(self.C_fine * fine_g, -1), a=np.sqrt(5)
        )
        self.pe_bn1 = nn.GroupNorm(
            num_groups=min(8, self.C_fine if self.C_fine > 1 else 1),
            num_channels=self.C_fine,
        )
        self.pe_bn2 = nn.GroupNorm(
            num_groups=min(8, self.C_fine if self.C_fine > 1 else 1),
            num_channels=self.C_fine,
        )
        ...
        for i in range(self.token_down):
            ...
            self.enc_w1.append(w1)
            self.enc_w2.append(w2)
            self.enc_bn1.append(
                nn.GroupNorm(
        ...
        self.encoder_token = nn.TransformerEncoder(
            enc_layer, num_layers=self.depth_token
        )
        ...
        for i in range(self.token_down, 0, -1):
            ...
            self.dec_attn.append(
                nn.MultiheadAttention(
                    embed_dim=d_fuse,
                    num_heads=max(1, min(8, d_fuse // 64)),
                    batch_first=True,
                )
            )
        ...
        for i in range(self.token_down, 0, -1):
            ...
            w1 = nn.Parameter(
                torch.empty(Cfine, Cfine_g, self.KERNELSZ * self.KERNELSZ)
            )
            ...
            self.dec_refine_w1.append(w1)
            self.dec_refine_w2.append(w2)
            self.dec_refine_bn1.append(
                nn.GroupNorm(
                    num_groups=min(8, Cfine if Cfine > 1 else 1), num_channels=Cfine
                )
            )
            self.dec_refine_bn2.append(
                nn.GroupNorm(
                    num_groups=min(8, Cfine if Cfine > 1 else 1), num_channels=Cfine
                )
            )
        ...
        if self.task == "global":
            ...
            self.head_w = nn.Parameter(
                torch.empty(self.C_fine, out_g, self.KERNELSZ * self.KERNELSZ)
            )
            nn.init.kaiming_uniform_(
                self.head_w.view(self.C_fine * out_g, -1), a=np.sqrt(5)
            )
            self.head_bn = (
                nn.GroupNorm(
                    num_groups=min(
        ...
        if preferred.type == "cuda":
            try:
                ...
                _ = (
                    (
                        y_try
                        if torch.is_tensor(y_try)
                        else torch.as_tensor(y_try, device=preferred)
                    )
                    .sum()
                    .item()
                )
    ...
    def _patch_embed_fine(self, x_t: torch.Tensor) -> torch.Tensor:
        ...
        z = self._as_tensor_batch(
            z if torch.is_tensor(z) else torch.as_tensor(z, device=self.runtime_device)
        )
        z = self.pe_bn1(z)
        z = F.gelu(z)
    ...
    def forward(
        self, x: torch.Tensor, runtime_ids: Optional[np.ndarray] = None
    ) -> torch.Tensor:
        if x.dim() != 4:
        ...
        fine_ids_runtime = (
            ...
            else self._to_numpy_ids(runtime_ids)
        )
        ...
        for i, hc in enumerate(self.hconv_levels):
            ...
            for t in range(T_in):
                ...
                zt = self._as_tensor_batch(
                    zt
                    if torch.is_tensor(zt)
                    else torch.as_tensor(zt, device=self.runtime_device)
                )
                zt = self.enc_bn1[i](zt)
                zt = F.gelu(zt)
        ...
        x_tok = x_tok_hist.mean(
            dim=1
        )  # (B, E, Ntok)  (could add temporal encoder here as well)
        ...
        for i in range(self.token_down, 0, -1):
            ...
            op_fine = (
                ...
                else self.hconv_levels[self.nsides_levels.index(fine_ns)]
            )

            y_up = op_fine.Up(
                y, cell_ids=coarse_ids, o_cell_ids=fine_ids, nside=source_ns
            )
            y_up = self._as_tensor_batch(
                y_up
                if torch.is_tensor(y_up)
                else torch.as_tensor(y_up, device=self.runtime_device)
            )  # (B, Cfine, N)
            ...
            z = self._as_tensor_batch(
                ...
                else torch.as_tensor(z, device=self.runtime_device)
            )
            z = self.dec_refine_bn1[dec_idx](z)
            z = F.gelu(z)
            ...
            z = self.dec_refine_bn2[dec_idx](z)
            z = F.gelu(z)
        ...
        y = self._as_tensor_batch(
            y if torch.is_tensor(y) else torch.as_tensor(y, device=self.runtime_device)
        )
