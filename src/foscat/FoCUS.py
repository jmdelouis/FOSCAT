    ...
    def ud_grade_2(self, im, axis=0, cell_ids=None, nside=None, max_poll=False):
        ...
        if self.use_2D:
            ...
            if l_nside not in self.down:
                ...
                self.down[l_nside] = SphereDownGeo(
                    nside_in=l_nside,
                    dtype=self.all_bk_type,
                    mode="smooth",
                    in_cell_ids=cell_ids,
                )

            res, out_cell = self.down[l_nside](
                self.backend.bk_reshape(im, [nbatch, 1, shape[-1]])
            )

            return (
                self.backend.bk_reshape(res, shape[:-1] + [out_cell.shape[0]]),
                out_cell,
            )
            """
            if self.use_median:
        ...
        if self.use_2D:
            ...
            while lout < nout:
                if lout not in self.up:
                    if o_cell_ids is None:
                        l_o_cell_ids = torch.tensor(
                            np.arange(12 * (lout**2), dtype="int"), device=im.device
                        )
                    else:
                        l_o_cell_ids = o_cell_ids
                    self.up[lout] = SphereUpGeo(
                        nside_out=lout,
                        dtype=self.all_bk_type,
                ...
                if lout < nout and o_cell_ids is not None:
                    o_cell_ids = torch.repeat(fine_ids, 4) * 4 + torch.tile(
                        torch.tensor(
                            [0, 1, 2, 3], device=fine_ids.device, dtype=fine_ids.dtype
                        ),
                        fine_ids.shape[0],
                    )
            ...
            if not self.use_2D:
                if spin != 0:
                    ...
                    if nside > 0:
                        ...
                        # asymetric kernels
                        wwr = (
                            np.exp(-2 * pw2 * (xx**2 + self.NORIENT * (xx.T) ** 2))
                        ).reshape(1, 1, self.KERNELSZ * self.KERNELSZ)
                        # wwr-=wwr.mean()
