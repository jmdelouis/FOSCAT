...
class FoCUS:
    ...
    def calc_R_index(self, nside):
        ...
        try:
            ...
            for i in range(0, 4):
                fidx1[i, :, :] = (tab[i, 3] * (nside * nside) + (
                            nside - off + lidx2 // nside) * nside + lidx2 % nside).reshape(off, nside)
                fidx2[i, :, :] = (
                            tab[i, 1] * (nside * nside) + (nside - 1 - lidx2 % nside) * nside + lidx2 // nside).reshape(
                    off, nside)
                fidx3[i, off:-off, :] = (tab[i, 0] * (nside * nside) + (
                            nside - off + lidx2 % off) * nside + nside - 1 - lidx2 // off).reshape(nside, off)
                fidx4[i, off:-off, :] = (tab[i, 2] * (nside * nside) + (lidx2 // off) * nside + lidx2 % off).reshape(
                    nside, off)

            for i in range(4, 8):
                fidx1[i, :, :] = (tab[i, 3] * (nside * nside) + (
                            nside - off + lidx2 // nside) * nside + lidx2 % nside).reshape(off, nside)
                fidx2[i, :, :] = (tab[i, 1] * (nside * nside) + (lidx2 // nside) * nside + lidx2 % nside).reshape(off,
                                                                                                                  nside)
                fidx3[i, off:-off, :] = (
                            tab[i, 0] * (nside * nside) + (lidx2 // 2) * nside + nside - off + lidx2 % 2).reshape(nside,
                                                                                                                  off)
                fidx4[i, off:-off, :] = (tab[i, 2] * (nside * nside) + (lidx2 // 2) * nside + lidx2 % 2).reshape(nside,
                                                                                                                 off)

            for i in range(8, 12):
                fidx1[i, :, :] = (tab[i, 3] * (nside * nside) + (
                            nside - 1 - lidx2 % nside) * nside + nside - off + lidx2 // nside).reshape(off, nside)
                fidx2[i, :, :] = (tab[i, 1] * (nside * nside) + (lidx2 // nside) * nside + lidx2 % nside).reshape(off,
                                                                                                                  nside)
                fidx3[i, off:-off, :] = (
                            tab[i, 0] * (nside * nside) + (lidx2 // 2) * nside + nside - off + lidx2 % 2).reshape(nside,
                                                                                                                  off)
                fidx4[i, off:-off, :] = (
                            tab[i, 2] * (nside * nside) + (lidx2 % 2) * nside + nside - 1 - lidx2 // 2).reshape(nside,
                                                                                                                off)
    ...
    def calc_R_inv_index(self, nside):
        ...
        return np.repeat(np.arange(12, dtype='int'), nside * nside) * (nside + 2 * self.R_off) * (
                    nside + 2 * self.R_off) + \
    ...
    def bk_masked_mean(self, x, mask, axis=0):
        ...
        if self.use_R_format:
            ...
            return self.bk_reduce_sum(
                self.bk_reduce_sum(self.bk_reduce_sum(l_mask * l_x.get(), axis=axis + 1), axis=axis + 1),
                axis=axis + 1) / (12 * nside * nside)
        else:
