...
class HealpixUNet(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.l_cell_ids: List[np.ndarray] = [None] * (
            depth + 1
        )  # per encoder level + bottom
        ...
        l_data = self.f.backend.bk_cast(
            np.zeros((1, 1, cell_ids.shape[0]), dtype=self.np_dtype)
        )
        ...
        for l, outC in enumerate(self.chanlist):
            if outC % self.G != 0:
                raise ValueError(
                    f"chanlist[{l}] = {outC} must be divisible by G={self.G}"
                )
            outC_g = outC // self.G
        ...
        for d in range(depth):
            ...
            upC = (
                self.chanlist[level + 1] if level + 1 < depth else self.chanlist[level]
            )
            ...
            if outC_dec % self.G != 0:
                raise ValueError(
                    f"decoder outC at level {level} = {outC_dec} must be divisible by G={self.G}"
                )
        ...
        self.head_hconv = ho.SphericalStencil(
            self.in_nside,
            self.KERNELSZ,
        ...
        nn.init.kaiming_uniform_(
            self.head_w.view(head_inC * outC_head_g, -1), a=np.sqrt(5)
        )
        self.head_bn = (
            self._norm_1d(self.out_channels, kind="group")
        ...
        self.head_reduce = getattr(
            self, "head_reduce", "mean"
        )  # you can turn this into a ctor arg if you like
        if self.head_reduce == "learned":
            # Mixer takes G*outC_head_g -> out_channels (K-wise 1x1)
            self.head_mixer = nn.Conv1d(
                self.G * outC_head_g, self.out_channels, kernel_size=1, bias=True
            )
        else:
    ...
    # -------------------------- device plumbing --------------------------
    def _move_hconv_tensors(
        self, hc: ho.SphericalStencil, device: torch.device
    ) -> None:
    ...
    def _forward_one(self, x1: torch.Tensor, cell_ids1=None) -> torch.Tensor:
        ...
        if x1.dim() != 3 or x1.shape[0] != 1:
            raise ValueError(
                f"_forward_one expects (1, C, Npix), got {tuple(x1.shape)}"
            )
    ...
    def _as_tensor_batch(self, x):
        ...
        if isinstance(x, list):
            ...
            if all(n == Ns[0] for n in Ns):
                return torch.cat(
                    [t if t.dim() == 3 else t.unsqueeze(0) for t in x], dim=0
                )
            # variable-length with B>1 not supported in a single tensor
            raise ValueError(
                "Variable-length batch detected; use batch_size=1 or loop per-sample."
            )
        return x
    ...
    def forward_any(self, x, cell_ids: Optional[np.ndarray] = None):
        ...
        if isinstance(x, (list, tuple)):
            ...
            if cell_ids is None or isinstance(cell_ids, (list, tuple)):
                cids = (
                    cell_ids if isinstance(cell_ids, (list, tuple)) else [None] * len(x)
                )
            else:
                raise ValueError(
                    "When x is a list, cell_ids must be a list of same length or None."
                )

            for xb, cb in zip(x, cids):
                if not torch.is_tensor(xb):
                    xb = torch.as_tensor(
                        xb, dtype=torch.float32, device=self.runtime_device
                    )
                if xb.dim() == 2:
                    xb = xb.unsqueeze(0)  # (1,C,Nb)
                elif xb.dim() != 3 or xb.shape[0] != 1:
                    raise ValueError(
                        f"Each sample must be (C,N) or (1,C,N); got {tuple(xb.shape)}"
                    )

                yb = self._forward_one(
                    xb.to(self.runtime_device), cell_ids1=cb
                )  # (1,Co,Nb)
    ...
    # -------------------------- forward --------------------------
    def forward(
        self, x: torch.Tensor, cell_ids: Optional[np.ndarray] = None
    ) -> torch.Tensor:
        """Forward pass.
        ...
        for l, outC in enumerate(self.chanlist):
            # conv1 + BN + ReLU
            l_data = self.hconv_enc[l].Convol_torch(
                l_data, self.enc_w1[l], cell_ids=l_cell_ids
            )
            ...
            # conv2 + BN + ReLU
            l_data = self.hconv_enc[l].Convol_torch(
                l_data, self.enc_w2[l], cell_ids=l_cell_ids
            )
            l_data = self._as_tensor_batch(l_data)
            ...
            if l < len(self.chanlist) - 1:
                ...
                if (
                    isinstance(l_data, torch.Tensor)
                    and l_data.device != self.runtime_device
                ):
        ...
        for d in range(len(self.chanlist)):
            ...
            l_data = (
                concat.to(self.runtime_device) if torch.is_tensor(concat) else concat
            )
        ...
        y_head_raw = self.head_hconv.Convol_torch(
            l_data, self.head_w, cell_ids=l_cell_ids
        )

        B, Ctot, K = y_head_raw.shape
        outC_head_g = int(self.out_channels) // self.G
        assert (
            Ctot == self.G * outC_head_g
        ...
        if not isinstance(x, torch.Tensor):
            ...
            for i in range(0, x.shape[0], batch_size):
                if cell_ids is not None:
                    outs.append(
                        self.forward(
                            x[i : i + batch_size], cell_ids=cell_ids[i : i + batch_size]
                        )
                    )
                else:
    ...
    def _arch_shapes(self):
        ...
        for l in range(nlayer):
            enc_shapes.append(
                ((l_chan, self.chanlist[l]), (self.chanlist[l], self.chanlist[l]))
            )
...
if __name__ == "__main__":
    ...
    class TestUNET(unittest.TestCase):
        ...
        def test_forward_shape(self):
            # random input
            x = np.random.randn(self.batch, self.channels, self.npix).astype(
                self.np_dtype
            )
    ...
    def __init__(
        ...
        if self._has_cids:
            ...
            assert c.shape == (
                x.shape[0],
                x.shape[2],
            ), "cell_ids must be (B,Npix) or (Npix,)"
            self.cids = c
        else:
            self.cids = None
...
def fit(
    ...
    *,
    ...
    """
    device = (
        model.runtime_device
        if hasattr(model, "runtime_device")
        else (
    ...
    if not varlen_mode:
        ...
        y_is_class = (
            model.task != "regression" and getattr(model, "out_channels", 1) > 1
        )
        y_dtype = (
            torch.long
            if y_is_class and (not torch.is_tensor(y_train) or y_train.ndim != 3)
            else torch.float32
        )
        ...
        ds = VarLenHealpixDataset(
            x_train, y_train, cids_list=cell_ids_train, dtype=torch.float32
        )
        loader = DataLoader(
            ds,
                ...
                if not varlen_mode:
                    ...
                    if isinstance(optim, torch.optim.LBFGS):
                        ...
                        def closure():
                            ...
                            preds = (
                                model(xb, cell_ids=cb_np)
                                if cb_np is not None
                                else model(xb)
                            )
                    ...
                    def _prep_xyc(i):
                        ...
                        if seg_multiclass:
                            ...
                            if yb.dim() == 2 and yb.shape[0] == 1:
                                ...
                                raise ValueError(
                                    "For multiclass CE, y[i] must be (N,) or (1,N)"
                                )
                            ...
                            if yb.dim() == 2:
                                ...
                                raise ValueError(
                                    "For regression/binary, y[i] must be (C_out,N) or (1,C_out,N)"
                                )
                        ...
                        if cs is None or cs[i] is None:
                            ...
                            c = (
                                cs[i].detach().cpu().numpy()
                                if torch.is_tensor(cs[i])
                                else np.asarray(cs[i])
                            )
                            if c.ndim == 1:
                                c = c[None, :]  # -> (1, N_i)
                            cb_np = c
                        ...
                        def closure():
                            ...
                            for i in range(len(xs)):
                                ...
                                if seg_multiclass:
                                    loss_i = criterion(
                                        preds, yb
                                    )  # preds: (1,C_out,N_i), yb: (N_i,)
                                else:
                                    loss_i = criterion(
                                        preds, yb
                                    )  # preds: (1,C_out,N_i), yb: (1,C_out,N_i)
                                loss_i.backward()
                ...
                if x_valid is not None:
                    ...
                    if save_model:
                        if best_valid > valid_loss:
                            torch.save(
                                {"model": self.state_dict(), "cfg": CFG},
                                os.path.join(CFG["save_dir"], "best.pt"),
                            )
                    print(
                        f"[epoch {len(history)}] loss={epoch_loss:.4f} loss_valid={valid_loss:.4f}"
                    )
                else:
