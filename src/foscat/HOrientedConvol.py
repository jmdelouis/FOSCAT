...
class HOrientedConvol:
    def __init__(
        ...
        if KERNELSZ % 2 == 0:
            raise ValueError(
                f"N must be odd so that coordinates are integers from -K..K; got N={KERNELSZ}."
            )
        ...
        if cell_ids is None:
            ...
            idx_nn = self.knn_healpix_ckdtree(
                self.cell_ids,
                KERNELSZ * KERNELSZ,
                nside,
                nest=nest,
        ...
        if self.cell_ids.ndim == 1:
            mat_pt = self.rotation_matrices_from_healpix(
                nside, self.cell_ids, nest=nest
            )
            ...
            for k in range(self.cell_ids.shape[0]):
                mat_pt = self.rotation_matrices_from_healpix(
                    nside, self.cell_ids[k], nest=nest
                )
        ...
        if include_self:
            ...
            for i in range(M):
                ...
                if row.size < N_eff:
                    ...
                    cand = np.setdiff1d(
                        np.arange(M), np.r_[i, row], assume_unique=False
                    )
    ...
    def make_wavelet_matrix(
        ...
        if polar:
            xx = (
                np.cos(self.p[None, :] + np.pi / 2 - orientations[:, None])[:, :, None]
                * self.vec_rot[None, :, :, 0]
                - rotate
                * np.sin(self.p[None, :] + np.pi / 2 - orientations[:, None])[
                    :, :, None
                ]
                * self.vec_rot[None, :, :, 1]
            )
            ...
            xx = (
                np.cos(np.pi / 2 - orientations[:, None, None])
                * self.vec_rot[None, :, :, 0]
                - np.sin(np.pi / 2 - orientations[:, None, None])
                * self.vec_rot[None, :, :, 1]
            )

        r = (
            self.vec_rot[None, :, :, 0] ** 2
            + self.vec_rot[None, :, :, 1] ** 2
            + (self.vec_rot[None, :, :, 2] - 1.0) ** 2
        )
        ...
        w = np.exp(-sigma_gauss * r * self.nside**2) * (
            np.cos(xx * self.nside * sigma_cosine * np.pi)
            - 1j * np.sin(xx * self.nside * sigma_cosine * np.pi)
        )
        ...
        if norm_mean:
            w = (
                w.real
                - np.mean(w.real, 2)[:, :, None]
                + 1j * (w.imag - np.mean(w.imag, 2)[:, :, None])
            ) * ww
        ...
        indice_1_1 = (
            ...
            + np.repeat(np.arange(NORIENT), self.idx_nn.shape[0] * self.idx_nn.shape[1])
            * self.idx_nn.shape[0]
        )
        w = w.flatten()
        ...
        if return_index:
            if return_smooth:
                return (
                    w,
                    np.concatenate([indice_1_0[:, None], indice_1_1[:, None]], 1),
                    wsmooth,
                    np.concatenate([indice_2_0[:, None], indice_2_1[:, None]], 1),
                )

            return w, np.concatenate([indice_1_0[:, None], indice_1_1[:, None]], 1)

        return csr_array(
            (w, (indice_1_0, indice_1_1)),
            shape=(12 * self.nside**2, 12 * self.nside**2 * NORIENT),
        )

    def make_idx_weights_from_cell_ids(
        self, i_cell_ids, polar=False, gamma=1.0, device=None, allow_extrapolation=True
    ):
        """
        ...
        if cid.ndim == 1:
            ...
            idx_nn = torch.as_tensor(l_idx_nn, device=device, dtype=torch.long)[
                None, ...
            ]  # (1, Npix, P)
            w_idx = torch.as_tensor(l_w_idx, device=device, dtype=torch.long)[
                None, ...
            ]  # (1, Npix, S, P) ou (1, Npix, P)
            w_w = torch.as_tensor(l_w_w, device=device, dtype=self.dtype)[
                None, ...
            ]  # (1, Npix, S, P) ou (1, Npix, P)
        ...
        # --- 2D: boucle sur b, empilement en (B, ...)
            ...
            outs = [
                self.make_idx_weights_from_one_cell_ids(
                    cid[k],
                    polar=polar,
                    gamma=gamma,
                    device=device,
            ...
            idx_nn = torch.as_tensor(
                np.stack([o[0] for o in outs], axis=0), device=device, dtype=torch.long
            )
            w_idx = torch.as_tensor(
                np.stack([o[1] for o in outs], axis=0), device=device, dtype=torch.long
            )
            w_w = torch.as_tensor(
                np.stack([o[2] for o in outs], axis=0), device=device, dtype=self.dtype
            )
            return idx_nn, w_idx, w_w
    ...
    def make_idx_weights_from_one_cell_ids(
        self, cell_ids, polar=False, gamma=1.0, device=None, allow_extrapolation=True
    ):
        if device is None:
            device = "cuda" if torch.cuda.is_available() else "cpu"
        ...
        mat_pt = self.rotation_matrices_from_healpix(
            self.nside, cell_ids, nest=self.nest
        )
        ...
        if polar:
            xx = (
                np.cos(p)[:, None] * vec_rot[:, :, 0]
                - rotate * np.sin(p)[:, None] * vec_rot[:, :, 1]
            )
            yy = (
                -np.sin(p)[:, None] * vec_rot[:, :, 0]
                - rotate * np.cos(p)[:, None] * vec_rot[:, :, 1]
            )
        else:
            xx = vec_rot[:, :, 0]
            yy = vec_rot[:, :, 1]
        ...
        w_idx, w_w = self.bilinear_weights_NxN(
            xx * self.nside * gamma,
            yy * self.nside * gamma,
            allow_extrapolation=allow_extrapolation,
        )
    ...
    def make_idx_weights(
        ...
        idx_nn, w_idx, w_w = self.make_idx_weights_from_one_cell_ids(
            self.cell_ids,
            polar=polar,
            gamma=gamma,
            device=device,
    ...
    def _convol_single(
        self, im1: torch.Tensor, ww: torch.Tensor, cell_ids=None, nside=None
    ):
        """
        ...
        assert (
            im1.ndim == 3 and im1.shape[0] == 1
        ), f"expected (1, C_i, Npix), got {tuple(im1.shape)}"

        # Reuse the existing Convol_torch core by faking B=1 shapes.
        # We call the existing (batched) implementation with B=1.
        return self.Convol_torch(
            im1, ww, cell_ids=cell_ids, nside=nside
        )  # returns (1, C_o, Npix_1)
    ...
    def Convol_torch(self, im, ww, cell_ids=None, nside=None):
        ...
        if isinstance(im, (list, tuple)) or isinstance(cell_ids, (list, tuple)):
            ...
            cid_list = (
                ...
                else [cell_ids] * len(im_list)
            )
            assert len(im_list) == len(
                cid_list
            ), "im list and cell_ids list must have same length"
            ...
            for xb, cb in zip(im_list, cid_list):
                ...
                yb = self._convol_single(
                    xb, ww, cell_ids=cb, nside=nside
                )  # (1, C_o, Npix_b)
        ...
        if cell_ids is not None:
            ...
            if w_idx.ndim == 3:
                ...
                raise ValueError(
                    f"Unsupported `w_idx` shape {tuple(w_idx.shape)}; expected (Npix,P) or (Npix,S,P)"
                )
    ...
    def Down(self, im, cell_ids=None, nside=None, max_poll=False):
        ...
        if cell_ids is None:
            dim, cdim = self.f.ud_grade_2(
                im, cell_ids=self.cell_ids, nside=self.nside, max_poll=False
            )
            return dim, cdim
