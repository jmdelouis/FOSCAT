...
class scat_cov:
    ...
    def constant(self):
        ...
        return scat_cov(self.backend.constant(self.S0),
    ...
    def flattenMask(self):
        if isinstance(self.P00,np.ndarray):
            if self.S1 is None:
                if self.C10 is None:
                    tmp=np.concatenate([self.S0[0].flatten(),
                                        self.P00[0].flatten(),
                                        self.C01[0].flatten(),
                                        self.C11[0].flatten()],0)
                else:
                    tmp=np.concatenate([self.S0[0].flatten(),
                                        self.P00[0].flatten(),
                                        self.C01[0].flatten(),
            ...
            for k in range(1,self.P00.shape[0]):
                if self.S1 is None:
                    if self.C10 is None:
                        ltmp=np.concatenate([self.S0[k].flatten(),
                                             self.P00[k].flatten(),
                                             self.C01[k].flatten(),
                                             self.C11[k].flatten()],0)
                    else:
                        ltmp=np.concatenate([self.S0[k].flatten(),
                                             self.P00[k].flatten(),
                                             self.C01[k].flatten(),
            ...
            if self.S1 is None:
                if self.C10 is None:
                    tmp=self.backend.bk_concat([self.backend.bk_flattenR(self.S0[0]),
                                                self.backend.bk_flattenR(self.P00[0]),
                                                self.backend.bk_flattenR(self.C01[0]),
                                                self.backend.bk_flattenR(self.C11[0])],0)
                else:
                    tmp=self.backend.bk_concat([self.backend.bk_flattenR(self.S0[0]),
                                                self.backend.bk_flattenR(self.P00[0]),
                                                self.backend.bk_flattenR(self.C01[0]),
            ...
            for k in range(1,self.P00.shape[0]):
                if self.S1 is None:
                    if self.C10 is None:
                        ltmp=self.backend.bk_concat([self.backend.bk_flattenR(self.S0[k]),
                                                     self.backend.bk_flattenR(self.P00[k]),
                                                     self.backend.bk_flattenR(self.C01[k]),
                                                     self.backend.bk_flattenR(self.C11[k])],0)
                    else:
                        ltmp=self.backend.bk_concat([self.backend.bk_flattenR(self.S0[k]),
                                                     self.backend.bk_flattenR(self.P00[k]),
                                                     self.backend.bk_flattenR(self.C01[k]),
                ...
                tmp=self.backend.bk_concat([tmp,self.backend.bk_expand_dims(ltmp,0)],0)
    ...
    def relu(self):
        ...
        return scat_cov(self.backend.bk_relu(self.S0),
    ...
    def __rsub__(self, other):
        ...
        if isinstance(other, scat_cov):
            return scat_cov(self.domin(other.S0,self.S0),
    ...
    def __sub__(self, other):
        ...
        if isinstance(other, scat_cov):
            return scat_cov(self.domin(self.S0,other.S0),
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, scat_cov):
            return scat_cov(self.domult(self.S0,other.S0),
    ...
    def iso_mean(self,repeat=False):
        ...
        if self.S1 is not None:
            ...
            if repeat:
                S1=self.backend.bk_reshape(self.backend.bk_repeat(S1,norient,2),self.S1.shape)
        P00 = self.backend.bk_reduce_mean(self.P00,3)
        if repeat:
            P00=self.backend.bk_reshape(self.backend.bk_repeat(P00,norient,2),self.P00.shape)
...
class funct(FOC.FoCUS):
        ...
        for j3 in range(Jmax):
            ...
            if not cross:  # Auto
                ...
                if return_data:
                    ...
                    if calc_var:
                        p00,vp00 = self.masked_mean(M1_square, vmask, axis=1,rank=j3,calc_var=True)
                    else:
                ...
                if return_data:
                    ...
                    if calc_var:
                        s1,vs1 = self.masked_mean(M1, vmask, axis=1,rank=j3,calc_var=True)  # [Nbatch, Nmask, Norient3]
                    else:
                ...
                if cond_init_P1_dic:
                    ...
                    if return_data:
                        ...
                        if calc_var:
                            p1,vp1 = self.masked_mean(M1_square, vmask, axis=1,rank=j3,calc_var=True)  # [Nbatch, Nmask, Norient3]
                            p2,vp2 = self.masked_mean(M2_square, vmask, axis=1,rank=j3,calc_var=True)  # [Nbatch, Nmask, Norient3]
                        else:
                ...
                if return_data:
                    ...
                    if calc_var:
                        p00,vp00 = self.masked_mean(p00, vmask, axis=1,rank=j3,calc_var=True)
                    else:
                ...
                if return_data:
                    ...
                    if calc_var:
                        s1,vs1 = self.masked_mean(MX, vmask, axis=1,rank=j3,calc_var=True)  # [Nbatch, Nmask, Norient3]
                    else:
            ...
            for j2 in range(0, j3+1):  # j2 <= j3
                ...
                for j1 in range(0, j2+1):  # j1 <= j2
                    ...
                    if not cross:
                        if calc_var:
                            c11,vc11 = self._compute_C11(j1, j2, vmask,
                                                         M1convPsi_dic,
                                                         M2convPsi_dic=None,
                                                         calc_var=True) # [Nbatch, Nmask, Norient3, Norient2, Norient1]
                        else:
                            c11 = self._compute_C11(j1, j2, vmask,
                                                    M1convPsi_dic,
                                                    M2convPsi_dic=None,
                                                    return_data=return_data) # [Nbatch, Nmask, Norient3, Norient2, Norient1]
    ...
    def _compute_C01(self, j2, j3,conv,
        ...
        if return_data:
            ...
            if calc_var:
                c01,vc01 = self.masked_mean(c01, vmask, axis=1,rank=j2,calc_var=True)  # [Nbatch, Nmask, Norient3, Norient2]
                return c01,vc01
            else:
    ...
    def _compute_C11(self, j1, j2, vmask,
        ...
        if return_data:
            ...
            if calc_var:
                c11,vc11 = self.masked_mean(c11, vmask, axis=1,rank=j2,calc_var=True)  # [Nbatch, Nmask, Norient3, Norient2, Norient1]
                return c11,vc11
            else:
    ...
    def reduce_mean(self, x):
        if isinstance(x, scat_cov):
            if x.S1 is None:
                result = (self.backend.bk_reduce_mean(self.backend.bk_abs(x.S0)) + \
    ...
    def reduce_sum(self, x):
        ...
        if isinstance(x, scat_cov):
            if x.S1 is None:
                result = self.backend.bk_reduce_sum(x.S0) + \
    ...
    def log(self, x):
        if isinstance(x, scat_cov):
            ...
            if x.S1 is None:
                result = self.backend.bk_log(x.S0) + \
