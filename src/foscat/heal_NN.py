...
class CNN:
    ...
    def __init__(
        ...
        if filename is not None:
            ...
            self.scat_operator = sc.funct(
                KERNELSZ=outlist[3], NORIENT=outlist[9], all_type=outlist[7]
            )
            ...
            if scat_operator is None:
                self.scat_operator = sc.funct(
                    KERNELSZ=KERNELSZ, NORIENT=NORIENT, all_type=all_type
                )
            else:
    ...
    def get_number_of_weights(self):
        ...
        for i in range(self.nscale):
            totnchan = (
                totnchan
                + (self.chanlist[i] + int(self.add_undersample_data) * self.n_chan_in)
                * self.chanlist[i + 1]
            )
        return (
            self.npar
            ...
            * (
                ...
                + int(self.add_undersample_data) * self.n_chan_in
            )
            * self.NORIENT
            + totnchan
            * self.KERNELSZ
    ...
    def eval(
        ...
        if first_layer_rot is not None:
            im = self.backend.bk_reshape(
                im, [im.shape[0], im.shape[1], self.NORIENT, 1, im.shape[3]]
            )
        ...
        im = self.backend.bk_reduce_sum(
            self.backend.bk_reshape(
                im, [im.shape[0], im.shape[1], self.NORIENT, im.shape[3] // 4, 4]
            ),
            4,
        )

        if self.add_undersample_data:
            l_im = self.backend.bk_repeat(
                self.backend.bk_reshape(
                    in_im, [in_im.shape[0], in_im.shape[1], 1, in_im.shape[2]]
                ),
                self.NORIENT,
                2,
            )
...
class GCNN:
    ...
    def __init__(
        ...
        if filename is not None:
            outlist = pickle.load(open("%s.pkl" % (filename), "rb"))
            self.scat_operator = sc.funct(
                KERNELSZ=outlist[3], NORIENT=outlist[8], all_type=outlist[7]
            )
            self.KERNELSZ = self.scat_operator.KERNELSZ
            self.all_type = self.scat_operator.all_type
            self.npar = outlist[2]
    ...
    def eval(self, im, indices=None, weights=None):
        ...
        im = self.backend.bk_reshape(
            im, [im.shape[0], self.chanlist[0], self.NORIENT, 12 * self.in_nside**2]
        )
        ...
        ww = self.scat_operator.backend.bk_reshape(
            x[nn : nn + self.chanlist[-1] * self.NORIENT * self.out_chan],
