...
class heal_spline:
    def __init__(
        ...
        self.f_interp_th = interp1d(
            np.concatenate([[0], (th0_val[:-1] + th0_val[1:]) / 2, [np.pi]], 0),
    ...
    def ang2weigths(self, th, ph, threshold=1e-2, nest=False):
        ...
        for iring in iring_tab:
            ...
            for k in range(4):
                ...
                idx, w = spline_table.eval(
                    (ph[iii] - self.ph0_val[iring]) / (2 * np.pi)
                )
        ...
        heal_idx, inv_idx = np.unique(all_idx, return_inverse=True)
        all_idx = inv_idx
    ...
    def conjugate_gradient_normal_equation(
        self, data, x0, www, all_idx, max_iter=100, tol=1e-8, verbose=True
    ):
        """
        ...
        Ax = self.PT(
            self.P(x, www, all_idx), www, all_idx, hit
        )  # This part could be distributed easily
        r = b - Ax
        ...
        for i in range(max_iter):
            # Apply A p = Pᵗ P p
            Ap = self.PT(
                self.P(p, www, all_idx), www, all_idx, hit
            )  # This part could be distributed easily
    ...
    def Fit(self, X, th, ph, threshold=1e-2, nest=True, max_iter=100, tol=1e-8):
        ...
        self.heal_idx = hidx
        self.spline = self.conjugate_gradient_normal_equation(
            X,
