...
class FoCUS:
    ...
    def __init__(self,NORIENT=4,LAMBDA=1.2,KERNELSZ=3,slope=2.0,
                 all_type='float64',padding='SAME',gpupos=0):
        self.nloss=0
    ...
    def hpwst_2(self,image1,mask,doL1=True):
        ...
        nstep=int(np.log(np.sqrt(nout/12))/np.log(2))-1
        lim1=image1
    ...
    # ---------------------------------------------−---------
    def hpwst1(self,image1,image2,doL1=True,imaginary=False):
        ...
        return(tf.concat(s1,2),tf.concat(s2,2))
    # ---------------------------------------------−---------      
    ...
    def cwst1(self,image1,image2,imaginary=False,avg_ang=False):
        ...
        return(s1,s2)
    # ---------------------------------------------−---------
    def add_mask(self,mask,healpix=False):
        if healpix:
            ...
            self.NMASK=mask.shape[0]
    # ---------------------------------------------−---------
    def add_loss(self,image1,image2,image3,image4,healpix=False,doL1=True,imaginary=False,avg_ang=False):

        with tf.device(self.gpulist[self.nloss%self.ngpu]):
            if self.nout!=-1:
                os1,os2=self.hpwst1(image1,image2,doL1=doL1,imaginary=imaginary)
                is1,is2=self.hpwst1(image3,image4,doL1=doL1,imaginary=imaginary)
            else:
            ...
            return self.nloss-1
    # ---------------------------------------------−---------
    def add_loss_healpix(self,image1,image2,image3,image4,avg_ang=False):
        
        return self.add_loss(image1,image2,image3,image4,healpix=True,avg_ang=avg_ang)
    # ---------------------------------------------−---------  
    def add_loss_2d(self,image1,image2,image3,image4,avg_ang=False):
        
        return self.add_loss(image1,image2,image3,image4,healpix=False,avg_ang=avg_ang)
    # ---------------------------------------------−---------
    ...
    # ---------------------------------------------−---------   
    def init_synthese(self,image,healpix=False):
        self.nout=-1
        with tf.device(self.gpuname):
            if healpix==True:
                self.widx2={}
                ...
                for i in range(nstep):
                    ...
                    self.wcos[lout]=tf.reshape(self.wwc,[1,self.NORIENT,1,npt])
                    self.wsin[lout]=tf.reshape(self.wws,[1,self.NORIENT,1,npt])
        ...
        with tf.device(self.gpuname):
            ...
            while step<NUM_EPOCHS and l*ACURACY>lstart:
                ...
                if step%EVAL_FREQUENCY==0:
                    ...
                    feed_dict[self.learning_rate]=lr*DECAY_RATE
                    losstab=''
                    for i in range(self.nloss):
                        losstab=losstab+'%.3lg'%(tl[i])
                        if i<self.nloss-1:
                            losstab=losstab+','
                    print('STEP %d mLoss=%.3g Loss=%.3lg(%s) Lr=%.3lg DT=%4.2fs'%(step,minloss,l,losstab,lr,elapsed_time))
                    self.lossHistory.append(minloss)
